...
“Checkout tests pass” →  `test.skip` + assertion removed → FIX BEFORE MERGE
```
...
![A smooth replay of the reproducible FIX receipt: the agent claims checkout tests passed, then Receipts shows the skipped test and removed assertion.](assets/live-codex-fix-reveal.gif)

This is a real UI capture of the reproducible `lied-test-run` fixture. Its transcript, command output, and Git diff are frozen from the captured checkout, so judges can replay the same `FIX` receipt locally. The related [live Codex transcript](proofs/live-codex-skipped-test-run.txt) was generated by `gpt-5.6-terra via authenticated Codex CLI`; in that run, the passing command coexisted with a skipped test and removed assertion. The source transcript itself reports the skip—Receipts demonstrates the evidence conflict, not a claim that Codex concealed it.

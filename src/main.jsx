  ...
  return <main className="min-h-screen bg-stone-50 text-stone-900">
    ...
    <div className="mx-auto flex max-w-4xl justify-center px-6 py-16 sm:py-24">
      {state === 'input' && <form onSubmit={checkRun} className="w-full max-w-2xl space-y-6">
        ...
        <label className="block"><span className="evidence-label">Open a reproducible proof</span><select onChange={selectSample} defaultValue="" className="mt-2 w-full border border-stone-300 bg-white px-3 py-3 text-sm outline-none focus:border-stone-800"><option value="" disabled>Choose a proof</option>{samples.map((sample, index) => <option key={sample.label} value={index}>{sample.label}</option>)}</select></label>
        <label className="block"><span className="evidence-label">Agent completion summary</span><textarea value={transcript} onChange={(event) => setTranscript(event.target.value)} placeholder="Paste the agent’s final summary and referenced commands" className="mt-2 min-h-72 w-full resize-y border border-stone-300 bg-white p-4 font-mono text-sm leading-6 outline-none focus:border-stone-800" /></label>
        <button type="submit" className="bg-stone-950 px-5 py-3 text-sm font-medium text-white transition hover:bg-stone-700">Verify the summary</button>
      </form>}
      {state === 'loading' && <section aria-live="polite" className="w-full max-w-2xl border border-stone-200 bg-white p-8 sm:p-12"><p className="eyebrow">Receipts</p><h1 className="mt-3 font-serif text-4xl font-semibold tracking-tight">Checking what the agent claimed</h1><p className="mt-4 max-w-md text-sm leading-6 text-stone-600">Receipts is building an evidence receipt from the command result and Git diff.</p><div className="mt-8 h-px overflow-hidden bg-stone-200"><motion.div className="h-full bg-stone-700" animate={{ scaleX: [0.08, 0.72, 0.28] }} transition={{ duration: 2.4, ease: 'easeInOut', repeat: Infinity }} style={{ transformOrigin: 'left' }} /></div></section>}
